# Documentation

## Deployment Guides

All deployment-related documentation is in the `deployment/` folder:

- [DEPLOYMENT_GUIDE.md](deployment/DEPLOYMENT_GUIDE.md) - Main deployment guide
- [DEPLOYMENT_MULTI_ENV.md](deployment/DEPLOYMENT_MULTI_ENV.md) - Multi-environment setup
- [DOCKER_DEPLOYMENT.md](deployment/DOCKER_DEPLOYMENT.md) - Docker deployment
- [PRODUCTION_DEPLOYMENT.md](deployment/PRODUCTION_DEPLOYMENT.md) - Production deployment
- [PRODUCTION_DEPLOYMENT_GUIDE.md](deployment/PRODUCTION_DEPLOYMENT_GUIDE.md) - Detailed production guide
- [CONTAINER_SYNC_DEPLOYMENT.md](deployment/CONTAINER_SYNC_DEPLOYMENT.md) - Sync container deployment
- [SYSTEMD_SERVICE_SETUP.md](deployment/SYSTEMD_SERVICE_SETUP.md) - Systemd service setup
- [DEPLOY_TO_REMOTE.md](deployment/DEPLOY_TO_REMOTE.md) - Remote deployment

## Feature Documentation

Feature-specific documentation is in the `features/` folder:

- [CLEANUP_FUNCTIONALITY.md](features/CLEANUP_FUNCTIONALITY.md) - Cleanup functionality
- [ID_ENTREPRISE_FEATURE.md](features/ID_ENTREPRISE_FEATURE.md) - Enterprise ID feature
- [SYNC_DEPLOYMENT.md](features/SYNC_DEPLOYMENT.md) - Sync service deployment

## Other

- [CLEANUP_SUMMARY.md](CLEANUP_SUMMARY.md) - Previous cleanup summary
- [TODO.md](TODO.md) - Development TODO list

## Quick Links

- **Main README**: [../README.md](../README.md)
- **Frontend README**: [../frontend/README.md](../frontend/README.md)
- **Frontend AGENT.md**: [../frontend/AGENT.md](../frontend/AGENT.md)
